...
- Showdown session behavior is important and should remain intact:
  ...
  - Capped permanent severe injuries use one count-based entry in the persistent survivor `severeInjuries` array; do not duplicate their limits into survivor JSON. Severe injury metadata owns those limits. Display filled/empty pips beside the injury name in both the severe table and the normal Create/View Survivor record. At a once/twice cap, replace `Apply` with the bleeding symbol and grant 1 bleeding token instead. Multiple-times injuries apply their concrete effects without being listed as persistent injury records. Keep textual limit values and the overflow explanation out of the player-facing reference modal.
  - Severe injury `Apply` actions must remain conservative and immediate, without a confirmation dialog: automate only fully represented deterministic effects. Dice, random selections, Fighting Arts interactions, other-survivor effects, and unresolved board state remain manual. An unsafe row may expose only its explicit bleeding-token action. Approved persistent reminders cover restrictions, retirement, and skipping the next hunt.
