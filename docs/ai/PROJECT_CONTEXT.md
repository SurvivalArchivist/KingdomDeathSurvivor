...
- Severe table rows show `Apply` only for fully deterministic app-owned effects. Permanent rows with additional unsafe/manual effects expose `Record` for the deterministic permanent portion and retain a separate bleeding-symbol action where appropriate; dice, random choices, Fighting Arts handling, other survivors, and unresolved board state remain manual. Create/View Survivor can add built-in permanent injuries and provides `Heal` per recorded occurrence. Adding applies only deterministic persistent effects; healing decrements/removes the record and reverses that occurrence's deterministic permanent stat and restriction effects. Neither action changes bleeding tokens or other temporary/manual consequences. Campaign severe-injury reminders use the authoritative saved settlement year: skip-next-hunt effects target the following Lantern Year, while retirement effects target the current Lantern Year. Vignette or unavailable settlement context retains the undated rule text.
...
- Weapon proficiency type is selected from an app-owned catalog rather than free text or private reference files. Normal proficiencies grant Specialization at rank 3 and Mastery at rank 8. Twilight Sword instead unlocks staged effects at ranks 2, 4, and 6 before Mastery at rank 8. Showdown's proficiency popover displays each built-in rule and its active/locked state.
...
- Application configuration is written durably through flushed temporary files and atomic rename to both `config.json.bak` and `config.json`. An unreadable primary is preserved with a timestamped `config.json.corrupt-*` name, restored from the last-known-good backup when possible, and reported in the renderer; when no valid backup exists, defaults are shown with an explicit warning rather than silently replacing the damaged file.
...
- LAN Client recovery messaging distinguishes unreachable host, request timeout, uncertain write outcome, validation failure, stale revision conflict, and generic server error. Ordinary Host reads time out after eight seconds; writes time out after fifteen seconds and are reported as potentially committed so the user refreshes authoritative data instead of blindly retrying. Auto Reconnect surfaces `Reconnecting` status while checking host health or restoring the live update stream. Stream registration has an eight-second acknowledgement timeout, automatic retries use jittered exponential backoff capped at 30 seconds and reset after registration/manual connection actions, and a deliberate Host stop sends a best-effort shutdown event so Clients transition offline promptly. After a known disconnect, system resume, app activation, or window focus retries immediately without duplicating healthy/currently connecting streams.
