...
- Capped permanent severe injuries persist through normal Showdown saves as one `{ location, name, count }` entry in the survivor's `severeInjuries` array. Limits remain canonical built-in metadata rather than being duplicated into survivor JSON. The severe table and normal Create/View Survivor record display filled and empty pips beside capped injury names. The table compares the persisted count with the metadata and changes `Apply` to the bleeding symbol at the once/twice cap. Multiple-times injuries apply their concrete effects without creating a persistent injury listing. Textual limit values and the overflow explanation stay hidden from the player-facing modal.
- Severe table rows show `Apply` only for deterministic app-owned effects. Actions apply immediately without a confirmation dialog and can update survivor stats, proficiency, alive state, temporary tokens, bleeding, capped severe-injury counts, and approved reminders for restrictions, retirement, or the next hunt. Results requiring dice, random choices, Fighting Arts handling, other survivors, or unresolved board state stay manual; when safe, those rows expose only a bleeding-symbol action for their explicit bleeding token.
